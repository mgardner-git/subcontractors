<%@page import="java.util.*" %>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>

<script type="text/javascript" src="/subcontractors/resources/js/angular.min.js"></script>
<script type="text/javascript" src="/subcontractors/resources/js/jquery-1.11.2.min.js"></script>
<link rel="stylesheet" type="text/css" href="/subcontractors/resources/js/jquery-ui-1.11.4.custom/jquery-ui.min.css"/>
<link rel="stylesheet" type="text/css" href="/subcontractors/resources/style/subcontractors.css"/>
<link rel="stylesheet" type="text/css" href="/subcontractors/resources/js/ngTooltips-master/css/ng-tooltips.css"/>
<link rel="stylesheet" type="text/css" href="/subcontractors/resources/js/jquery-ui-timepicker-addon.css"/>
<script type="text/javascript" src="/subcontractors/resources/js/jquery-ui-1.11.4.custom/jquery-ui.min.js"></script>
<script type="text/javascript" src="/subcontractors/resources/js/jquery-ui-timepicker-addon.js"></script>
<script type="text/javascript" src="/subcontractors/resources/js/subcontractors.js"></script>
<script type="text/javascript" src="/subcontractors/resources/js/ngTooltips-master/js/ngtooltips.js"></script>
<script type="text/javascript" src="/subcontractors/resources/js/directives/date-time.js"></script>
<title>Subcontractors</title>
</head>
<body ng-app="subcontractorsApp" ng-controller="subcontractorsController">
<div id="wrap">
	<header>
	</header>
	<jsp:include page="topNav.jsp"/>
    <div id="leftColumn">
    	<jsp:include page="nav.jsp"/>
    </div>
    <div id="content">
    
    
    
    